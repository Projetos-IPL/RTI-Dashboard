...
function SensorCard({ sensorType }) {
  ...
  const [latestSensorLogRecord: SensorLog, setLatestSensorLogRecord] = useState(new SensorLog(null, null, null, null));
  ...
  useEffect(() => {
    getDataWithAuthToken(API_ROUTES.SENSOR_LOG_API_ROUTE, {
      ...
      .then((res) => {
        ...
        let latestLog = new SensorLog(data.sensor_log_id, data.sensor_id, data.name, data.value, data.timestamp);
    ...
    return (
      <Card>
        <Card.Body>
          ...
          <h6>
            Valor: <span className='fw-normal'>{latestSensorLogRecord.value}</span>
          </h6>
